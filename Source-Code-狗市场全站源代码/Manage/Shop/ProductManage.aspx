...
<div class="row">
    <ol id="BreadNav" class="breadcrumb fixed-top">
        ...
        <div class="pull-right ml-auto">
            <span>
                <span onclick="showstore(this);" class="fa z01-icon139" title="按店铺筛选" > </span>
                ...
                <ItemTemplate>
                    <div class="Ctitle sys_userPopver" onmouseover="ShowPopover(this)">
                        ...
                        <div class="popover bs-popover-right popbox">
                            <div class="arrow"></div>
                            <h3 class="popover-header">商品概览<a href="javasvript:;" class="hidePopover"><i class="fa fa-close" aria-hidden="true"></i></a></h3>
...
<!--                    <a class="precode" href="javascript:;" <%#Eval("istrue").ToString().Equals("1") ? "":"hidden" %> data-href="<%#GetShopUrl() %>"><i class="fa fa-qrcode"></i></a>-->
					...
					<a href='ShowProduct.aspx?id=<%# Eval("ID") %>' title="预览"> <i class="fa fa-eye"></i> </a>
					<a  href='<%#Eval("Class","").Equals("2") ? "Arrive/SuitProAdd.aspx?id="+Eval("ID") : "AddProduct.aspx?id="+Eval("id") %>'> <i class="fa fa-pencil" title="<%=Resources.L.编辑 %>"></i>编辑</a>
					 <a  href="<%#GetShopUrl() %>" target="_blank" title="<%=Resources.L.浏览 %>"> <i class="fa fa-globe"></i>浏览</a>
					 <a  href="javascript:;" onclick="showStock(<%#Eval("ID") %>);"><i class="fa fa-bank"></i>库存</a>
					 	
					<button type="button" class="btn btn-link sys_precode_btn" data-toggle="tooltip" data-html="true" title="<div class='sys_precode'><img src='/common/common.ashx?url=<%Call.Label("{$SiteURL/}"); %>/shop/<%# Eval("ID") %>.aspx' alt=''/><div> "><i class="fa fa-qrcode"></i></button>
                    <asp:LinkButton runat="server"  CommandName="Del1" CommandArgument='<%# Eval("id") %>' OnClientClick="return confirm('确定要将商品移入回收站吗');"><i class="fa fa-trash-o" title="<%=Resources.L.删除 %>"></i></asp:LinkButton>
