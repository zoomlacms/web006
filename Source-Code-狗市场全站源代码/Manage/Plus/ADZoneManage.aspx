    ...
    <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="ZoneID" PageSize="20" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="">
                <ItemTemplate>
                    <a href="Advertisement.aspx?ZoneId=<%# Eval("ZoneID") %>" ><i class="fa fa-plus" title="添加"></i> 发布</a>
                    <a href="ADZone.aspx?ZoneId=<%# Eval("ZoneID") %>"><i class="fa fa-pencil" title="修改"></i> 编辑</a>
                    ...
                    <asp:LinkButton ID="LinkButton5" runat="server" CommandName="Clear" OnClientClick="return confirm('确定清空此版位？');" CommandArgument='<%# Eval("ZoneID") %>'><i class="fa z01-icon217" title="清空"></i> 清空</asp:LinkButton>
                    <asp:LinkButton ID="LinkButton6" runat="server" CommandName="SetAct" CommandArgument='<%# Eval("ZoneID") %>'><i class="fa fa-pause" aria-hidden="true"></i> <%# GetUnActive(DataBinder.Eval(Container.DataItem, "Active").ToString())%></asp:LinkButton>
