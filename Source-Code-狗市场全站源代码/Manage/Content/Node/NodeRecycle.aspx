...
<ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="20" IsHoldState="false" 
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="">
            <ItemTemplate>
                <asp:LinkButton runat="server"  CommandName="reconver" CommandArgument='<%#Eval("NodeID") %>'><i class="fa fa-recycle" title=""></i>恢复</asp:LinkButton>
                <asp:LinkButton runat="server"  CommandName="del2" CommandArgument='<%#Eval("NodeID") %>' OnClientClick="return confirm('确定要删除吗');"><i class="fa fa-trash-o"></i>删除</asp:LinkButton>
            </ItemTemplate>
        </asp:TemplateField>
    </Columns>
</ZL:ExGridView>
